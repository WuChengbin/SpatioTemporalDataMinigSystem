...
{
    public partial class FormRoSTCM : Form
    {
        string DataSetName = "";
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                DataSetName = HDF4Operator.GetDatasetsName(inFileNames[0].ToString());
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            {
                MaxIgnoredValue = double.Parse(textBoxMax.Text);
                MinIgnoredValue = double.Parse(textBoxMin.Text);
            }
            ...
            int Neighborhood = 8;
            Neighborhood = int.Parse(comboBoxNeighborhood.SelectedItem.ToString());
            ...
            for (int i = 0; i < FileNum; i++)
            {
                ...
                for (int j = 0; j < row * col; j++)
                {
                    ...
                    rsets.Attribute = psBuffer[j + i * row * col];
                    rsets.x = j / col;
                    rsets.y = j % col;
                    rsets.t = i;
                    rsets.a = 0;
            ...
            for (int j = 0; j < FileNum * row * col; j++)
            {
                ...
                else if (Rasterpixels[j].t >= T && Rasterpixels[j].t <= FileNum - T-1)//else if(Rasterpixels[j].t >=2 && Rasterpixels[j].t <=mFileNum-3 && Rasterpixels[j].t%2 ==0)
                {
                    ...
                    for (int r = 0; r < arrvalrasters.Count(); r++)
                    {
                        ...
                        if (desDr.rsID == Rasterpixels[j].rsID - T * row * col)//正上方和正下方的邻域也是其最近邻
                        {
                            c = 1;
                        }
                        if (desDr.rsID == Rasterpixels[j].rsID + T * row * col)
                        {
                            d = 1;
                        }
                    }

                    if (Rasterpixels[j].Getneighborgrids().Count() >= CP && c == 1 && d == 1)//原值为20
            ...
            //删小簇
            int minnum = 100;//猜想：改为（8*((mRows*mCols)/20000)*mFileNum）更好！
