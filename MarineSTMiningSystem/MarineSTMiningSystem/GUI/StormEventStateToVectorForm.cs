...
{
    public partial class StormEventStateToVectorForm : Form
    {
        ...
        public StormEventStateToVectorForm(OracleConnection _conn)
        {
            InitializeComponent();
        ...
        public StormEventStateToVectorForm(string connString)
        {
            InitializeComponent();
        ...
        {//后台工作方法
            ...
            //string sql = "SELECT table_name, column_name, data_type FROM all_tab_cols WHERE table_name = '"+tableName+"'";
            //DataTable dtResult = QueryResultTable(sql);
            ...
            for (int timeId = 0; timeId < startTimes.Rows.Count; timeId ++)
            {
                ...
                threads[tId] = new Thread(new ParameterizedThreadStart(StormToShpTime));
                threads[tId].IsBackground = true;
        ...
        private void StormToShpTime(object obj)
        {
            ...
            Layer olayer1 = ds1.CreateLayer(p._startTime.ToString("yyyyMMdd_HHmmss"), sr, wkbGeometryType.wkbPolygon, null);//接下来创建属性表字段
            // 先创建一个叫FieldID的整型属性
            ...
            // 先创建一个叫FieldID的整型属性
            FieldDefn oFieldStormStateID = new FieldDefn("StateID", FieldType.OFTString);
            ...
            //创建Power字段
            FieldDefn oFieldValue = new FieldDefn("Power", FieldType.OFTInteger);
            ...
            string sql = "select oid,event_id,state_id,time,sdo_geometry.get_wkt(space),min_log,min_lat,max_log,max_lat,area,avg_rain,volume,max_rain,min_rain,power,core_log,core_lat from " + stateTableName + " where time= to_date('" + p._startTime.ToString("yyyyMMdd HHmmss")+ "','yyyymmdd hh24:mi:ss')";
            DataTable dtResult = QueryResultTable(sql, orcConns[p._threadId]);

            //创建
            FieldDefn oFieldCoreLog = new FieldDefn("CoreLog", FieldType.OFTReal);
            ...
            for (int id = 0; id < dtResult.Rows.Count; id++)
            {
                ...
                oFeature.SetField("StormID", Convert.ToInt32(dtResult.Rows[id][1]));
                oFeature.SetField("StateID", (dtResult.Rows[id][2]).ToString());
                if(outParentsChildren)
                {//导出父子节点关系
                ...
                oFeature.SetField("MinLog", Convert.ToDouble(dtResult.Rows[id][5]));

                oFeature.SetField("MinLat", Convert.ToDouble(dtResult.Rows[id][6]));
                oFeature.SetField("MaxLog", Convert.ToDouble(dtResult.Rows[id][7]));
                oFeature.SetField("MaxLat", Convert.ToDouble(dtResult.Rows[id][8]));
                oFeature.SetField("Area", Convert.ToDouble(dtResult.Rows[id][9]));

                oFeature.SetField("AvgRain", Convert.ToDouble(dtResult.Rows[id][10]));
                oFeature.SetField("Volume", Convert.ToDouble(dtResult.Rows[id][11]));
                oFeature.SetField("MaxRain", Convert.ToDouble(dtResult.Rows[id][12]));
                oFeature.SetField("MinRain", Convert.ToDouble(dtResult.Rows[id][13]));
                oFeature.SetField("Power", Convert.ToDouble(dtResult.Rows[id][14]));
                ...
                oFeature.SetField("CoreLog", Convert.ToDouble(dtResult.Rows[id][15]));
                oFeature.SetField("CoreLat", Convert.ToDouble(dtResult.Rows[id][16]));
                //"POLYGON ((0 0,20 0,10 15,0 0),(5 2,10 10,15 2,5 2))"
                string polygonStr = dtResult.Rows[id][4].ToString();
        ...
        private void StormEventStateToVectorForm_Load(object sender, EventArgs e)
        {
