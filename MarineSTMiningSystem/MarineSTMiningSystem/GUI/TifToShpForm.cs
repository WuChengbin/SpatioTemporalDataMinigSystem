...
{
    public partial class TifToShpForm : Form
    {
        ...
        {//后台工作方法
            for(int i=0;i<spFileNames.Count;i++)
            {
                ...
                string idPath = idFileNames[i].ToString();//编号图像路径
                ...
                Parameter p = new Parameter(oriPath, valueScale, timeCell, spPath, idPath, outPath);
                threads[tId].Start(p);
        ...
        {//参数结构体
            ...
            public Parameter(string _oriPath, double _valueScale, double _timeCell, string _spPath, string _idPath, string _outPath) : this()
            {
                this._oriPath = _oriPath;
                this._valueScale = _valueScale;
                this._timeCell = _timeCell;
                this._spPath = _spPath;
                this._idPath = _idPath;
        ...
        private void TifToShp(object obj)
        {
            ...
            ResterToVector.TifToShp(p._oriPath, p._valueScale, p._timeCell, p._spPath, p._idPath, p._outPath);
        }
        ...
        ListBox.ObjectCollection idFileNames;
        double valueScale;
        double timeCell;
        //double maxTimeInv;
        ...
        private void okBtn_Click(object sender, EventArgs e)
        {
            ...
            if(!(listBox1.Items.Count== listBox2.Items.Count&& listBox2.Items.Count== listBox3.Items.Count))
            {//三个数目不同
