...
{
    public partial class StormProcessExtractForm : Form
    {
        ...
        public StormProcessExtractForm(OracleConnection _conn)
        {
            InitializeComponent();
        ...
        public StormProcessExtractForm(string connString)
        {
            InitializeComponent();
        ...
        private void SaveToRelatTB(StormPolygon stormPolyPrior, StormPolygon stormPolyNext, int tId)
        {
            ...
            foreach(StormPolygon poly in stormPolyPrior.childList)
            {//噪声
            ...
            foreach (StormPolygon poly in stormPolyNext.parentList)
            {//噪声
            ...
            {//当前时间每个暴雨链
                ...
                {//不是第一个
                    ...
                    {//上一时刻每个多边形
                        ...
                        if (SOD >= sodMinValue)
                        {//添加链接关系
                            if(stormPolygonLast.fileId==8&& stormPolygonLast.id==2)
                            {
                                int a = 0;
                            }
        ...
        private void PolyToList(object obj)
        {
            ...
            foreach (StormPolygon stormPolygon in stormPolyList)
            {
                ...
                {//没有父，即为头
                    StormPolygonList stormPolygonList = new StormPolygonList(stormPolygon);//StormList的头即为该多边形
                    stormPolygonList.GetList();//获取头的子，包括子的子
                    if(stormPolygonList.stormPolygons.Count==1)
                    {//只有一个点
                        stormPolygonList.stormPolygons[0].isAcnode = true;
                    }
        ...
        private void PolyConnect(object obj)
        {
            ...
            {//每个前多边形
                ...
                {//每个后多边形
                    if (stormPolyListPre[_r].fileId == 8 && stormPolyListPre[_r].id == 2&& stormPolyListNext[_c].fileId == 9 && stormPolyListNext[_c].id == 7)
                    {
                        int a = 0;
                    }
            ...
            {//遍历每一个对应关系
                ...
                {//不是添加的行列
                    ...
                    {
                        //bool isConnected = false;//默认还没有进行连接
                        //foreach (StormPolygon child in stormPolyListPre[prePos].childList)
                        //{
                        //    if (child.id == stormPolyListNext[nextpos].id)
                        //    {
                        //        isConnected = true;
                        //        break;
                        //    }
        ...
        private void ReadShp(object obj)
        {
            ...
            string startTime = Path.GetFileName(filePath).Substring(0, 6) + "01000000";
            string endTime = Path.GetFileName(filePath).Substring(0, 6) + "28000000";
            //Shp shp = new Shp(filePath);//读取shp
            StormShp stormShp = new StormShp(filePath);//读取shp
            stormShp.fileId = fileId;//记录文件id，方便进行时间上的查询
                                     //字符串转时间
            stormShp.startTime = DateTime.ParseExact(startTime, "yyyyMMddHHmmss", System.Globalization.CultureInfo.CurrentCulture);//起始时间
            stormShp.endTime = DateTime.ParseExact(endTime, "yyyyMMddHHmmss", System.Globalization.CultureInfo.CurrentCulture);//结束时间
        ...
        {//工作完成方法
            ...
            {
                //string sql = "delete from event_state_relation";
