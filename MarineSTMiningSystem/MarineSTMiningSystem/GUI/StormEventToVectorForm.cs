...
{
    public partial class StormEventToVectorForm : Form
    {
        ...
        public StormEventToVectorForm()
        {
            InitializeComponent();
        }

        public StormEventToVectorForm(OracleConnection _conn)
        {
            InitializeComponent();
        ...
        public StormEventToVectorForm(string connString)
        {
            InitializeComponent();
            worker.WorkerReportsProgress = true;
            worker.WorkerSupportsCancellation = true;//支持取消
        ...
        {//后台工作方法
            ...
            for (int startOid= minOid; startOid <= maxOid; startOid+=1000)
            {
                ...
                threads[tId] = new Thread(new ParameterizedThreadStart(StormToShpRange));
                threads[tId].IsBackground = true;
        ...
        private void StormToShp(object obj)
        {
            Parameter p = (Parameter)obj;
            string sql = "select oid,id,start_time,end_time,duration,sdo_geometry.get_wkt(space),min_log,min_lat,max_log,max_lat,avg_rain,volume,max_rain,min_rain,power from " + tableName + " where oid=" + Convert.ToInt32(p._oid);
            ...
            Layer olayer1 = ds1.CreateLayer("Storm_" + result[1].ToString(), sr, wkbGeometryType.wkbPolygon, null);
            //接下来创建属性表字段
            // 先创建一个叫FieldID的整型属性
            ...
            FieldDefn oFieldStartTime = new FieldDefn("StartTime", FieldType.OFTString);
            ...
            oFeature.SetField(2, result[2].ToString());
            oFeature.SetField(3, result[3].ToString());

            oFeature.SetField(4, Convert.ToDouble(result[4]));

            oFeature.SetField(5, Convert.ToDouble(result[6]));
            oFeature.SetField(6, Convert.ToDouble(result[7]));
            oFeature.SetField(7, Convert.ToDouble(result[8]));
            oFeature.SetField(8, Convert.ToDouble(result[9]));

            oFeature.SetField(9, Convert.ToDouble(result[10]));
            oFeature.SetField(10, Convert.ToDouble(result[11]));
            oFeature.SetField(11, Convert.ToDouble(result[12]));
            oFeature.SetField(12, Convert.ToDouble(result[13]));
            oFeature.SetField(13, Convert.ToDouble(result[14]));
            //"POLYGON ((0 0,20 0,10 15,0 0),(5 2,10 10,15 2,5 2))"
            string polygonStr = result[5].ToString();
        ...
        private void StormToShpRange(object obj)
        {
            ...
            string wkt = "GEOGCS[\"WGS 84\",DATUM[\"WGS_1984\",SPHEROID[\"WGS 84\",6378137,298.257223563,AUTHORITY[\"EPSG\",\"7030\"]],AUTHORITY[\"EPSG\",\"6326\"]],PRIMEM[\"Greenwich\",0],UNIT[\"degree\",0.0174532925199433],AUTHORITY[\"EPSG\",\"4326\"]]";//自定义投影坐标系的WKT
            OSGeo.OSR.SpatialReference sr = new OSGeo.OSR.SpatialReference(wkt);
            Layer olayer1 = ds1.CreateLayer("Storm_Oid" + p._startOid+"-"+p._endOid, sr, wkbGeometryType.wkbPolygon, null);//接下来创建属性表字段
            // 先创建一个叫FieldID的整型属性
            FieldDefn oFieldID = new FieldDefn("OID", FieldType.OFTInteger);
            olayer1.CreateField(oFieldID, 1);
            ...
            {
                string sql = "select oid,id,start_time,end_time,duration,sdo_geometry.get_wkt(space),min_log,min_lat,max_log,max_lat,avg_rain,volume,max_rain,min_rain,power from " + tableName + " where oid=" + Convert.ToInt32(oid);
                DataTable dtResult = QueryResultTable(sql, orcConns[p._threadId]);
                DataRow result = dtResult.Rows[0];
                //写入数据
                FeatureDefn oDefn = olayer1.GetLayerDefn();
